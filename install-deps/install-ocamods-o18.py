...
''' This script gets modules from OCA repository on GitHub to version 13
    ...
    * You can use it on Debian/Ubuntu as root user or sudo way:
        ...
        chmod +x install-ocamods-o18.py
        ./install-ocamods-o18.py

'''
import os, re, sys

paqrepdeb = "git"
git_url = 'https://github.com/OCA/'
branch = '18.0'
repositories = [
    'account-analytic',
    'account-budgeting',
...
if not os.path.exists('oca'):
    ...
    for r in repositories:
        os.system('cd oca; git clone -b %s %s%s' % (branch, git_url, r))
else:
    for r in repositories:
        if not os.path.exists('oca/%s' % r):
            os.mkdir('oca/%s' % r)
            os.system('cd oca; git clone -b %s %s%s; cd -' % (branch, git_url, r))
        else:
